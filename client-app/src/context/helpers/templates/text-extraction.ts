...
export function createTextExtractionTemplate({
  setNodes,
  ...
  const inputNode = createInputNode({
    ...
    event,
    setEdges,
    additionalAttributes: { template: TEMPLATES.TEXT_EXTRACTION_TEMPLATE },
  })
...
export function createTextExtractionNode({ screenToFlowPosition, setNodes, putCustomNode, event }: any) {
  return createNode({
    screenToFlowPosition,
    setNodes,
    putCustomNode,
    ...
    type: NODE_TYPES.TEXT_EXTRACTION,
  })
}
